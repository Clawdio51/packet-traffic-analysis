...
df = pd.read_csv(r"Data\data_youtube_wifi.csv", delimiter=',')
#df = pd.read_csv(r"Data\data_youtube_claudio.csv", delimiter=',')
...
for x in range(start,end):
    ...
    elif list_source.iloc[x]=="66.130.90.142":
    #elif list_source.iloc[x]=="178.135.6.12":
...
plt.hist(arrival_time_source[range(0,number_source)],bins=300,range=(0.0000001,0.003),label="Arrival time")
#plt.title('Arrival time of packets from the source')
#plt.show()

# Plot of departure time
plt.hist(arrival_time_destination[range(0,number_destination)],bins=300,range=(0.0000001,0.003),label="Departure time")
plt.title('Departure and arrival time of packets')
plt.legend(loc="upper right")
plt.savefig('exp1_packet_time_hist_wifi', format='eps')
plt.show()
