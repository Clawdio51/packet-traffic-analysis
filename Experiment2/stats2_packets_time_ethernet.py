...
df = pd.read_csv(r"Data\data_zoom_ethernet.csv", delimiter=',')
...
for x in range(start,end):
    if list_source.iloc[x]=="192.168.0.111":
    ...
    elif list_source.iloc[x]=="184.162.145.57":
...
plt.hist(arrival_time_source[range(0,number_source)],bins=300,range=(-0.0000001,0.00125),label="Arrival time")
#plt.title('Arrival time of packets from the source')
#plt.show()

# Plot of departure time
plt.hist(arrival_time_destination[range(0,number_destination)],bins=300,range=(-0.0000001,0.00125),label="Departure time")
plt.title('Departure and arrival time of packets')
plt.legend(loc="upper right")
plt.savefig('exp2_packettime_hist_ethernet.eps', format='eps')
plt.show()
